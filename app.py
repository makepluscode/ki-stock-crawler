...
class KoreaInvestmentAPI:
    def __init__(self):
        ...
        if not self.api_key or not self.api_secret:
            raise ValueError("API_KEY와 API_SECRET이 .env 파일에 설정되어 있지 않습니다.")
    ...
    def format_ranking_data(self, data):
        ...
        if 'output' not in data:
            raise ValueError(f"Unexpected API response format. Available keys: {data.keys()}")
        ...
        # 숫자형 데이터 변환
        numeric_columns = ['현재가', '전일대비', '등락율', '거래량', '거래대금',
                          '시가', '고가', '저가', '시가총액', '상장주식수',
                          '외국인보유비율', 'PER', 'EPS', 'PBR', 'BPS']
